...
const createCoupon = asyncHandler(async (req, res) => {
  ...
  if (duplicateCoupon) {
    throw new ApiError(
      409,
      "Coupon with code" + duplicateCoupon.code + "already exists."
    );
  }
...
const applyCoupon = asyncHandler(async (req, res) => {
  ...
  const clientCart = await getCart(req.user._id);

  if (clientCart.cartTotal < coupon.minimumCartValue) {
    throw new ApiError(
      400,
      "Add items worth TK " +
        (coupon.minimumCartValue - clientCart.cartTotal) +
...
const updateCoupon = asyncHandler(async (req, res) => {
  ...
  if (duplicateCoupon[0]) {
    throw new ApiError(
      409,
      "Coupon with code " + duplicateCoupon[0].code + " already exists"
    );
  }
